...
def seleccionar_opcion(max_value, text):
    opcion = 0
    while True:
        os.system("cls")
        try:
            opcion = int(input(f"{text}\nSeleccione una opción: "))
            if 1 <= opcion <= max_value:
                return opcion
            else:
                input("La opción ingresada no existe, presione ENTER para continuar...")
        except:
            input("El dato ingresado debe ser un número, presione ENTER para continuar...")
